...
feature {NONE} -- part 1
	g (p: POINT_STATE): INTEGER
		do
			if path_cost_g.has (p) then
				Result := path_cost_g.definite_item (p)
			else
				Result := {INTEGER}.Max_value
			end
		end
	...
	f (p: POINT_STATE): INTEGER
		do
			if path_cost_f.has (p) then
				Result := path_cost_f.definite_item (p)
			else
				Result := {INTEGER}.Max_value
			end
		end
	...
	point_with_best_f (points: HASH_TABLE [INTEGER, POINT_STATE]): POINT_STATE
		...
		do
			...
			--Result := points.first
			create Result.make (start, Right, 0)
			across points as pc
			loop
				tmp := f (pc.key)
				if tmp < best then
	...
	a_star
		...
		do
			...
			create curr.make (start, Right, 0)
